...
public class CarController :MonoBehaviour
{
   ...
   private float originalMaxMotorTorque; // Store the original maximum motor torque

    // Method to apply a torque boost to the car
    public void ApplyTorqueBoost(float amount, float duration)
    {
        StartCoroutine(Boost(amount, duration));
    }

    private IEnumerator Boost(float amount, float duration)
    {
        // Apply additional torque for the specified duration
        originalMaxMotorTorque = MaxMotorTorque; // Store the original maximum motor torque
        MaxMotorTorque += amount; // Increase the maximum motor torque by the boost amount
        yield return new WaitForSeconds(duration);
        // Reset the torque back to its original value
        MaxMotorTorque = originalMaxMotorTorque;
    }
